...
describe("EIP712Counter test", function () {
  ...
  before(async function () {
    ...
    deployerAccount = availableSigners[0];
  ...
  it("Should allow a gas relayer send a transaction on behalf some other account", async function () {
    ...
    const deployerEthBeforeTx = await deployerAccount.getBalance();

    const signatureMessage = {
      signer: deployerAccount.address,
    ...
    const signedMessage = await ethers.provider.send(
      ...
      [deployerAccount.address, typedData],
    );
    ...
    const lastStoredMessageForAccount = await counterTmpInstance.lastMessageOf(deployerAccount.address);
    // Getting the message count for the user
    const messageCountForAccount = await counterTmpInstance.countOf(deployerAccount.address);
    ...
    const deployerEthAfterTx = await deployerAccount.getBalance();
